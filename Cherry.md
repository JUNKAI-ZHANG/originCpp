...
constexpr ll P = 31525197391593473ll;
vector<ll> rev, roots{0, 1};

inline ll mul(ll a, ll b) {
    return (lll)a * (lll)b % P;
}
...
struct Poly {
    ...
    Poly(ll a0) { if (a0) a = {a0}; }
    Poly(const vector<ll> &a1) : a(a1) {
        while(!a.empty() && !a.back()) a.pop_back();
    }
    int size() const { return a.size(); }
    ll operator[](int idx) const { if (idx < 0 || idx >= size()) return 0; return a[idx]; }
    Poly mulxk(ll k) const {
        auto b = a;
        b.insert(b.begin(), k, 0);
        return Poly(b);
    }
    Poly mulInt(ll x) const {
        auto b = a;
    ...
    friend Poly operator*(Poly a, Poly b) {
        ...
        dft(a.a);
        dft(b.a);
        for (int i = 0; i < sz; ++i) a.a[i] = mul(a[i], b[i]);
        idft(a.a);
        return Poly(a.a);
    }
    ...
    Poly deriv() const {
        ...
        for (int i = 0; i < size() - 1; ++i) res[i] = mul(i + 1, a[i + 1]);
        return Poly(res);
    }
    Poly integr() const {
        if(a.empty()) return Poly();
        vector<ll> res(size() + 1);
        for (int i = 0; i < size(); ++i) res[i + 1] = mul(a[i], power(i + 1, P - 2));
        return Poly(res);
    }
    ...
    vector<ll> eval(vector<ll> x) const {
        if (size() == 0) return vector<ll>(x.size(), 0);
...
```shell
#!/bin/bash
g++-11 -o main main.cpp -std=c++17
g++-11 -o std std.cpp -std=c++17
g++-11 -o data data.cpp -std=c++17
while true; do
    ./data > data.in
    ./std <data.in > std.out
    ./main <data.in > my_out.out
    if diff std.out my_out.out; then
        printf "AC\n"
    else
        printf "Wa\n"
        exit 0
